...
def _get_short_cache(section: str, user_id: str | None, limit: int, ttl_seconds: int | float | None = None) -> Any | None:
    entry = SECTION_CACHE.get(_cache_key(section, user_id, limit))
    if not entry:
        return None
    ttl = SHORT_CACHE_TTL_SECONDS if ttl_seconds is None else ttl_seconds
